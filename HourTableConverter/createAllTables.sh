...
users=$(((eval "$(find '.cache/users.json' -amin -999 | grep -q .)" && cat '.cache/users.json')\
    ...
    | jq '.ocs.data.users[]')
#users=$(curl -su "bot:$(cat ../nc_api_bot_password.cred)" -H OCS-APIRequest:true -H Accept:application/json -X get https://warmulla.kaleidox.de/ocs/v1.php/cloud/users\
#    | jq '.ocs.data.users[]')
